...
namespace POS.Forms
{
    public partial class Pago : BaseContext
    {
                    ...
                    foreach (var item in detalleFactura)
                    {
                        if (_formPadre.ValidarExistencias(item, true))
                        {
                            ...
                            if (item.ProductoId > 0)
                            {
                                ...
                                detalle = NuevoDetalleFactura(producto, item, guidFacturaid);
                            }

                            if (item.DetalleRecargaId > 0)
                            {
                                var detallerecarga = _recargasRepository.GetDetalleRecarga(item.DetalleRecargaId);
                                detalle = DetalleFacturaRecarga(item,  guidFacturaid);
                            }
        ...
        private DetalleFactura NuevoDetalleFactura(Producto producto, ListarDetalleFacturas item, Guid idfact)
        {
            ...
            if (item.EstiloId != 0)
            {
                detalle.DetalleEstiloId = item.EstiloId;
            }
            if (item.ProductoId == 0)
            {
        ...
        private DetalleFactura DetalleFacturaRecarga(ListarDetalleFacturas item, Guid idfact)
        {
            var detalle = new DetalleFactura()
            {
                FacturaId = idfact,
                Descripcion = item.Descripcion,
                DetalleRecargaId = item.DetalleRecargaId,
                Cantidad = item.Cantidad,
                Precio = item.Precio,
                Descuento = item.Descuento,
                SubTotal = (item.Cantidad * item.Precio),
        ...
        private TemporalProductos NuevoProductosTemp(Producto producto, ListarDetalleFacturas item)
        {
            ...
            if (item.EstiloId != 0)
            {
                temporalP.DetalleEstiloId = item.EstiloId;
            }
            if (item.ProductoId == 0)
            {
        ...
        private TemporalProductos DetalleTemporalRecarga(ListarDetalleFacturas item)
        {
            var detalle = new TemporalProductos()
            {
                Descripcion = item.Descripcion,
                DetalleRecargaId = item.DetalleRecargaId,
                Cantidad = item.Cantidad,
        ...
        private void btnpagar_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    if (dialog == DialogResult.Yes)
                    {
                        ...
                        foreach (var item in detalleFactura)
                        {
                            if (_formPadre.ValidarExistencias(item, false))
                            {
                                ...
                                else if (item.EsRecarga)
                                {
                                    var producTemps = DetalleTemporalRecarga(item);
                                    _productosTempRepository.Add(producTemps, true);
                                }
                                else 
                                {
                ...
                _formPadre.ActualizarDocumentos();
            }
            catch (Exception ex)
            {
